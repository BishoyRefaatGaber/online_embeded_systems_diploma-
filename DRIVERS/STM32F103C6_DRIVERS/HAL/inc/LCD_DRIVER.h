...
#define LCD_INPUT_PINS      0xFFFFFFF0		//Zeros on the input pins of the LCD to be masked with the input
#define PORT_OUTPUT_PINS 	0x0000000F		//ones on the output pins of the port to be masked with the output to the LCD

#define MOST_BIT_PIN	4			////used to shift right the LCD input if it is in the middle of the Port  specially in 4 bit mode
#define LEAST_BIT_PIN	0				//used to shift left the LCD input if it is in the middle of the Port specially in 4 bit mode


#define RS  	GPIO_PIN_4		//The register select pin
#define WR  	GPIO_PIN_5		//read write pin
#define EN  	GPIO_PIN_6		//enable pin
//#define BF_PIN 	GPIO_PIN_7	//busy flag pin


//MICROCONTROLLER CONFIGURATIONS
#define LCD_DATA 			GPIOA->ODR		//Output data register of GPIO to LCD
#define LCD_CTRL            GPIOA->ODR		//Output control register of LCD  (RS , WR , EN) pins
...
//LCD CONTROL
#define LCD_ENABLE                   LCD_CTRL |= (1<<EN); wait_ms(1); LCD_CTRL &= ~(1<<EN); wait_ms(1);	//Enable of the LCD
...
#define LCD_WRITE_DATA                     (~(1<<WR) & (1<<RS))		//Write data from the GPIO TO LCD configurations
#define LCD_WRITE_INSTRUCTION             ~((1<<WR) | (1<<RS))		//Write instruction from the GPIO to LCD configurations
...
#define BAISX   128		//Base x position in the LCD
#define BAISY   64		//Base y position in the LCD
